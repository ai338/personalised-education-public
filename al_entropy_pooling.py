...
from functions_active_learning import retrain_parameters, entropy_reveal_one_question_per_student
...
bs_entropy1, bs_entropy2, bs_entropy3, bs_entropy4 = [], [], [], []
for it in range(iterations):
    labelled, unlabelled = entropy_reveal_one_question_per_student(params, labelled, unlabelled, max_score)
    labelled_data, labelled_data_max_scores = labelled[0], labelled[1]

    params, loss_entropy = retrain_parameters(labelled_data, labelled_data_max_scores, params, max_score, loss_entropy, it)
    ...
    bs_entropy1.append(bs_pool[1].clone().detach().numpy())
    bs_entropy2.append(bs_pool[2].clone().detach().numpy())
    bs_entropy3.append(bs_pool[3] .clone().detach().numpy())
    bs_entropy4.append(bs_pool[4].clone().detach().numpy())
    
    if it % 10 == 0:
        print(it)

torch.save(accs_entropy, os.path.join(os.getcwd(), 'al_params', 'entropy_pooling_accs_long_training.pth'))
torch.save(loss_entropy, os.path.join(os.getcwd(), 'al_params', 'entropy_pooling_loss_long_training.pth'))
torch.save(bs_entropy1, os.path.join(os.getcwd(), 'al_params', 'entropy_pooling_bs1.pth'))
torch.save(bs_entropy2, os.path.join(os.getcwd(), 'al_params', 'entropy_pooling_bs2.pth'))
torch.save(bs_entropy3, os.path.join(os.getcwd(), 'al_params', 'entropy_pooling_bs3.pth'))
torch.save(bs_entropy4, os.path.join(os.getcwd(), 'al_params', 'entropy_pooling_bs4.pth'))

print('Done with executing the script for entropy pooling')